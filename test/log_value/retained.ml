...
let consume
    ...
    run =
  [%log.trace "metadata"
    ~trace:(Delator.Field.int (trace [@log_value.trace]))
    ~debug:(Delator.Field.int (debug [@log_value.debug]))];
  run
...
let run run =
  ...
  let
    {
      run = _packet_run;
      trace = (packet_trace [@log_value.trace]);
      debug = (packet_debug [@log_value.debug]);
    } = packet
  in
  let C
        ( choice_run,
          (choice_trace [@log_value.trace]),
          (choice_debug [@log_value.debug]) ) = choice
  in
  [%log.trace "aggregate metadata"
    ~packet_trace:(Delator.Field.int (packet_trace [@log_value.trace]))
    ~packet_debug:(Delator.Field.int (packet_debug [@log_value.debug]))
    ~choice_trace:(Delator.Field.int (choice_trace [@log_value.trace]))
    ~choice_debug:(Delator.Field.int (choice_debug [@log_value.debug]))];
  consume
