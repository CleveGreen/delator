...
let instrument_expression ~name ~level ~skip_all ~log_exn expression =
  let rec instrument fields name_only expression =
    let loc = expression.pexp_loc in
    match expression.pexp_desc with
    | Pexp_fun (label, default, pattern, body) ->
        let converted = parameter_field pattern in
        let fields =
          match converted.field with
          | Some field when not skip_all && not name_only -> fields @ [ field ]
          | Some _ | None -> fields
        in
        { expression with
          pexp_desc =
            Pexp_fun
              (label, default, converted.pattern,
               instrument fields name_only body) }
    | Pexp_newtype (type_name, body) ->
        { expression with
          pexp_desc = Pexp_newtype (type_name, instrument [] true body) }
    | Pexp_function cases ->
        let cases =
          List.map
            (fun case ->
              { case with
                pc_rhs =
                  in_span ~loc:case.pc_rhs.pexp_loc ~name ~level ~fields:[]
                    ~log_exn case.pc_rhs })
            cases
        in
        { expression with pexp_desc = Pexp_function cases }
    | _ -> in_span ~loc ~name ~level ~fields ~log_exn expression
  in
  instrument [] false expression
