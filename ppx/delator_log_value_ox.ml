...
let formal_level pattern =
  ...
  let type_level =
    match pattern.ppat_desc with
    | Ppat_constraint (_, Some typ, _) -> level_on_type typ
    | _ -> None
  in
...
let strip_pattern_formal_level pattern =
  ...
  | Ppat_constraint (nested, Some typ, modes) ->
      ...
      { pattern with
        ppat_desc =
          Ppat_constraint
            (nested, Some { typ with ptyp_attributes = attributes }, modes) }
  | _ -> pattern
...
let constructor_levels constructor =
  ...
  | Pcstr_tuple arguments ->
      Some (List.map (fun argument -> level_on_type argument.pca_type) arguments)
  | Pcstr_record _ -> None
...
let tuple_levels typ =
  ...
  | Ptyp_tuple components ->
      Some (List.map (fun (_, component) -> level_on_type component) components)
  | _ -> None
...
let rec removable_pattern_is_irrefutable pattern =
  ...
  | Ppat_alias (nested, _) | Ppat_constraint (nested, _, _) ->
  ...
  object (self)
    ...
    method private tuple_expression_provenance expression =
      ...
      | Pexp_tuple components ->
          let levels =
            List.map
              (fun (_, component) -> level_on_expression component)
              components
          in
            ...
            | Pstr_type (_, declarations) ->
                List.iter
                  (fun declaration ->
                            ...
                            | Pcstr_tuple arguments ->
                                constructors :=
                                  ( constructor.pcd_name.txt,
                                    List.map
                                      (fun argument ->
                                        level_on_type argument.pca_type)
                                      arguments )
                                  :: !constructors
                            | Pcstr_record labels ->
                                List.iter
                    ...
                    | Ptype_open | Ptype_record_unboxed_product _ -> ())
                  declarations
            | _ -> ());
            loop rest
      in
    ...
    method! value_binding binding =
      ...
      let declared_provenance =
        Option.map (fun levels -> (levels, false))
          (Option.bind (tuple_type_name_on_pattern binding.pvb_pat)
    ...
    method! expression expression =
      ...
      | Pexp_let (_, rec_flag, bindings_here, body) ->
          let marked =
      ...
      | Pexp_function (parameters, constraint_, body) ->
          ...
          let additions =
            List.concat_map
              ...
              parameters
          in
          self#function_constraint constraint_;
          let shadowed =
            List.concat_map
              (fun parameter ->
                match parameter.pparam_desc with
        ...
        when !structural_components ->
          self#validate_tuple_type type_path
            (List.map
               ...
               components);
          List.iter
            (fun (_, component) -> self#expression_component component)
            components
    ...
    method! pattern pattern =
      ...
      | Ppat_record (record_fields, _) ->
          ...
          super#pattern pattern
      | Ppat_constraint
          ({ ppat_desc = Ppat_tuple (components, _); _ },
           Some
             ({ ptyp_desc = Ptyp_constr ({ txt = type_path; _ }, []); _ }
             as typ),
           _) ->
          self#validate_tuple_type type_path
            (List.map
               ...
               components);
          self#core_type typ;
          List.iter (fun (_, component) -> self#pattern component) components
      | Ppat_construct ({ txt = constructor; _ }, Some (_, argument)) -> (
          match argument.ppat_desc with
          | Ppat_tuple (components, _) ->
              self#validate_constructor_pattern constructor
...
let tuple_component_abi (label, component) =
  Contract.abi_component "tuple-component"
    [ Option.value ~default:"" label;
      contract_name (level_on_type component) ]
  ...
  let descriptor =
    ...
    | Ptype_variant constructors ->
        let constructor_abi constructor =
          let result =
            if Option.is_some constructor.pcd_res then "gadt" else "regular"
          in
          match constructor.pcd_args with
          | Pcstr_tuple arguments ->
              Contract.abi_component "constructor-tuple"
                ...
                :: List.map
                     (fun argument ->
                       contract_name (level_on_type argument.pca_type))
                     arguments)
          | Pcstr_record labels ->
    ...
    | Ptype_abstract -> (
        ...
        | Some { ptyp_desc = Ptyp_tuple components; _ } ->
            Some
              (Contract.abi_contract "tuple" (List.map tuple_component_abi components))
        | Some _ | None -> None)
    | Ptype_open -> Some (Contract.abi_contract "open" [])
    | Ptype_record_unboxed_product _ -> None
  in
  Option.map
    (fun contract ->
      { marker =
          ...
          List.fold_left
            ...
            (List.mapi
               (fun index level ->
                 (Contract.Positional (index + 1), level))
               levels))
  | Ptype_open | Ptype_record_unboxed_product _ -> contracts
...
class rewriter ~static_level ~binding_only_pattern_components =
  object (self)
    ...
    method! signature signature =
      ...
      { signature with
        psg_items =
          ...
          @ List.map
              (fun contract ->
                Contract.signature_contract ~loc:contract.loc
                  ~name:contract.marker ~contract:contract.contract)
              contracts }
        ...
        let args =
          ...
          | Pcstr_tuple arguments ->
              Pcstr_tuple
                (List.filter_map
                   (fun argument ->
                     match level_on_type argument.pca_type with
                     | Some level when not (self#keep level) -> None
                     | Some _ | None ->
                         Some
                           { argument with
                             pca_type = self#core_type argument.pca_type })
                   arguments)
        in
          ...
          let components =
            List.filter_map
              (fun (label, component) ->
                ...
                | Some level when not (self#keep level) -> None
                | Some _ | None ->
                    Some (label, self#core_type component))
              components
          in
          (match components with
          | [] ->
              ptyp_constr ~loc:typ.ptyp_loc
                { txt = Lident "unit"; loc = typ.ptyp_loc }
                []
          | [ _, component ] -> component
          | components -> { typ with ptyp_desc = Ptyp_tuple components })
      | Ptyp_arrow (label, argument, result, argument_modes, result_modes) ->
    ...
    method! expression expression =
      ...
      | None -> (
          match expression.pexp_desc with
          | Pexp_let (_mutable_flag, Nonrecursive, [ binding ], body) -> (
              match level_on_binding binding with
                    ...
                    List.filter_map
                      (fun (label, component) ->
                        Option.map
                          ...
                          (self#expression_component component))
                      components
                  in
                  let argument =
                    match components with
    ...
    method! pattern pattern =
      ...
      | None -> (
                  ...
                  let argument =
                    ...
                    | components ->
                        Some
                          ( vars,
                            { argument with
                              ppat_desc = Ppat_tuple (components, closed) } )
                  in
          ...
          | Ppat_tuple (components, closed) ->
              ...
              | [ _, component ] -> component
              | components ->
                  { pattern with ppat_desc = Ppat_tuple (components, closed) })
          | _ -> super#pattern pattern)
  end
