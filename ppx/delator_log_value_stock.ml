...
let validate_instrumentation_formals binding =
  let rec formals expression =
    match expression.pexp_desc with
    | Pexp_fun (_, _, pattern, body) -> pattern :: formals body
    | Pexp_newtype (_, body) -> formals body
    | _ -> []
  in
        ...
        (fun pattern ->
          ...
          | Some value_level
            when not (skipped_pattern pattern)
                 && not (compatible ~context:instrument_level value_level) ->
              Location.raise_errorf ~loc:pattern.ppat_loc
          ...
          | Some _ | None -> ())
        (formals binding.pvb_expr)

let function_signature expression =
  let rec loop reversed expression =
    match expression.pexp_desc with
    | Pexp_fun (label, _, pattern, body) ->
        loop ((label, formal_level pattern) :: reversed) body
    | Pexp_newtype (_, body) -> loop reversed body
    | Pexp_function _ -> List.rev ((Nolabel, None) :: reversed)
    | _ -> List.rev reversed
  in
  match expression.pexp_desc with
  | Pexp_fun _ | Pexp_newtype _ | Pexp_function _ -> Some (loop [] expression)
  | _ -> None
  ...
  object (self)
    ...
    method private validate_constructor_pattern path patterns =
      let components =
        List.map
          (fun pattern ->
            (pattern.ppat_loc, level_on_pattern pattern))
          patterns
      in
      let name = final_longident_name path in
      match
          ...
          self#with_bindings ~shadowed additions (fun () ->
              self#with_callables ~shadowed [] (fun () ->
                  let tail =
                    ...
                    | Pexp_fun _ | Pexp_newtype _ -> body.pexp_loc.loc_ghost
                    | _ -> false
                  in
                  with_ref function_tail tail (fun () -> self#expression body)))
      | Pexp_newtype (_, body) ->
          if not !function_tail then
            reject_fully_gated_callable ~loc:expression.pexp_loc
              (function_signature expression);
                  ...
                  pexp_desc =
                    Pexp_fun
                      ( label,
                        ...
                        self#pattern (strip_pattern_formal_level pattern),
                        self#expression body ) }
          | Pexp_newtype (name, body) ->
              { expression with
                pexp_desc = Pexp_newtype (name, self#expression body) }
          | Pexp_function cases ->
              { expression with pexp_desc = Pexp_function (List.map self#case cases) }
