...
let strip_function_parameter_attributes expression =
        ...
        List.map
          (fun parameter ->
            ...
            | Pparam_val (label, default, pattern) ->
                { parameter with
                  pparam_desc = Pparam_val (label, default, strip_pattern pattern) })
          parameters
      in
      { expression with pexp_desc = Pexp_function (parameters, constraint_, body) }
  | _ -> expression
...
let in_span ~loc ~name ~level ~fields ~log_exn body =
  let stack expression = { expression with pexp_desc = Pexp_stack expression } in
  let fields_thunk =
    stack (pexp_fun ~loc Nolabel None (punit ~loc) (elist ~loc fields))
  in
  let body_thunk = stack (pexp_fun ~loc Nolabel None (punit ~loc) body) in
  let application =
  ...
  in
  let nontail =
    { attr_name = { txt = "nontail"; loc }; attr_payload = PStr [];
      attr_loc = loc }
  in
  { application with pexp_attributes = nontail :: application.pexp_attributes }
